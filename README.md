...
<img src="./res/logo.png" width="96" height="96" alt="安禾播放器 Logo" />

# 安禾播放器

一款跨平台、插件化、可定制的桌面音乐播放器。
...
安禾播放器是一个基于 Electron 的桌面音乐播放器，支持 Windows、macOS 和 Linux。它专注于本地数据管理、插件化音源解析和界面定制，不内置广告，也不会上传你的个人数据。
...
| 功能           | 说明                                                       |
| -------------- | ---------------------------------------------------------- |
| 跨平台桌面端   | 支持 Windows、macOS 和 Linux                               |
| 插件化播放解析 | 支持通过洛雪 `.js` 插件解析播放链接                        |
| 音源重定向     | 可将歌曲播放请求重定向到可用解析源                         |
| 多音质切换     | 支持低品、标准、高品、超品等音质选项，具体可用性取决于插件 |
| 主题包         | 支持通过主题包自定义颜色、背景和界面观感                   |
| 本地数据       | 应用数据保存在本机，个人数据不会被上传                     |

## 插件

安禾播放器支持安装洛雪 `.js` 插件文件，或以 `.js` 结尾的 http(s) 插件链接。

内置插件文件为根目录下的 [`changqing_chajian.js`](./changqing_chajian.js)，启动时会作为内置洛雪插件加载。插件声明的 source 会被适配为播放器中的播放解析源，可用于音源重定向。
